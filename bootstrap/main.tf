...
resource "ibm_is_instance" "bootstrap_node" {
  count          = var.destroy_bootstrap ? 0 : 1
  ...
  primary_network_interface {
    ...
    security_groups = concat(var.control_plane_security_group_id_list, [ibm_is_security_group.bootstrap.0.id])
  }
...
resource "ibm_is_floating_ip" "bootstrap_floatingip" {
  count = local.public_endpoints ? (var.destroy_bootstrap ? 0 : 1) : 0

  name           = "${local.prefix}-bootstrap-node-ip"
  resource_group = var.resource_group_id
  target         = ibm_is_instance.bootstrap_node.0.primary_network_interface.0.id
  tags           = local.tags
}
...
resource "ibm_is_security_group_rule" "bootstrap_ssh_inbound" {
  count     = var.destroy_bootstrap ? 0 : 1
  group     = ibm_is_security_group.bootstrap.0.id
  direction = "inbound"
...
resource "ibm_is_lb_pool_member" "kubernetes_api_public" {
  ...
  target_address = ibm_is_instance.bootstrap_node.0.primary_network_interface.0.primary_ipv4_address
}
